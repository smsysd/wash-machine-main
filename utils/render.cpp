...
void _init_ledmatrix() {
	...
	int addr = JParser::getf(_config, "address", "display");
	int dp = JParser::getf(_config, "dir-pin", "display");
	string driver = JParser::getf(_config, "driver", "display");
	int brn = JParser::getf(_config, "baud-rate", "display");
	_handlerDelay = JParser::getf(_config, "handler-delay", "display");
	_redrawBorehole = JParser::getf(_config, "redraw-borehole", "display");
	int br;
	if (brn == 2400) {
		br = B2400;
	} else
	...
	try {
		...
		cout << "fail to hardware init: " << e.what() << endl;
		sleep(10);
		goto lminit;
	}
